...
public class Main {
    ...
    static String mainHeroName() {
        ...
        for (int i = 0; i < 5; i++) { // Делаем проверку не более 5 раз, дабы не зацикливать цикл.
            char[] tempArray = scanner.nextLine().toCharArray(); // Превращяем строку в массив чаров, чтоб потом проверить каждый чар
        ...
        if (resultUserName.length() == 1) {
            ...
            System.out.printf("Ваше имя: %ы",resultUserName);
